...
run custom DML queries, consider creating a view. You can now define
...
2.  Run custom DML queries from backend microservices instead of
...
### Args syntax
  ...
  check\_metadata\_consistency   false      Boolean   When set to `false`, the sql is executed without checking metadata dependencies.
  read\_only                     false      Boolean   When set to `true`, the query will be run in `READ ONLY` transaction access mode which means only `select` queries will be successful. This flag ensures that the GraphQL schema is not modified and is hence highly performant.
...
The response is a JSON Object with the following structure.
  ...
  -------------- ---------------- ------------------------------------------------------------- -------------------------------------------------------------
  result\_type   true             String                                                        One of \"CommandOk\" or \"TuplesOk\"
