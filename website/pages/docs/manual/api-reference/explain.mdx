...
The Explain API is used to analyse queries and subscriptions. It returns
a list of Postgres plans for a query and a single Postgres plan for a
...
:   -   `query`: the GraphQL query to be analysed
    -   `user` (optional): the `x-hasura-role` along with session
        variables
...
The Explain API is part of the
`Metadata API <metadata_apis>`{.interpreted-text role="ref"} and can
