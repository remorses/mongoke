...
take a look at the
`cli-migrations <auto_apply_migrations>`{.interpreted-text role="ref"}
Docker image, which can start the GraphQL engine after automatically
importing a mounted metadata directory.
