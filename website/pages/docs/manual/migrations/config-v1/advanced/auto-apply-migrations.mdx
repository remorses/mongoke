...
`config.yaml`) can be mounted at the `/hasura-migrations` path of this
Docker container and the container\'s entry point script will apply the
migrations before starting the server. If no directory is mounted at the
designated path, the server will start ignoring migrations.

If you want to mount the migrations directory at some location other
than `/hasura-migrations`, set the following environment variable:

```bash
HASURA_GRAPHQL_MIGRATIONS_DIR=/custom-path-for-migrations
```

Once the migrations are applied, the container resumes operation as a
normal Hasura GraphQL engine server.
...
this image to apply only the metadata, mount a directory with just a
`metadata.yaml` file and the script will apply the metadata.
