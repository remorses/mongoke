...
For `config v1`, see `auto_apply_migrations_v1`{.interpreted-text
role="ref"}.
...
this image to apply only the metadata, mount the `metadata` directory of
your Hasura project at the `/hasura-metadata` path of this Docker
