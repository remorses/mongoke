...
data <custom_views>`{.interpreted-text role="ref"} and then use
subscriptions to fetch the derived value and get its latest value
whenever it updates.
...
Next, `setup relationships <relationships>`{.interpreted-text
role="ref"} `poll` and `option` between the `poll_results` view and the
`poll` and `option` tables using the `poll_id` and `option_id` fields
respectively.
