...
of `queries <queries>`{.interpreted-text role="ref"} hold true for
...
By default updates are delivered to clients every **1 sec**. This
interval can be configured via the
`HASURA_GRAPHQL_LIVE_QUERIES_MULTIPLEXED_REFETCH_INTERVAL` env var or
the `--live-queries-multiplexed-refetch-interval` flag. See the
`server flag reference <server_flag_reference>`{.interpreted-text
role="ref"} for info on setting the flag/env var.

You can read more about the implementation of subscriptions in the
[architecture
doc](https://github.com/hasura/graphql-engine/blob/master/architecture/live-queries.md).
...
-   `subscribe_field`{.interpreted-text role="ref"}
-   `subscribe_table`{.interpreted-text role="ref"}
-   `subscribe_derived`{.interpreted-text role="ref"}
...
library for sending and receiving events.
...
3.  If CORS is disabled, the default behaviour is that the cookie won\'t
    ...
    `server_flag_reference`{.interpreted-text role="ref"} for the
    setting.
