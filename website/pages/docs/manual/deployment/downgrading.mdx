...
The Hasura GraphQL engine maintains its metadata state in a
\"catalogue\" as described
`here <hasura_metadata_schema>`{.interpreted-text role="ref"}. The
schema of the catalogue is versioned. Updates to the Hasura GraphQL
engine may have Hasura catalogue version bumps.
