...
-   `Hasura actions <actions>`{.interpreted-text role="ref"} are
    supported in Postgres 10 and above.
...
-   (required) Read & write access on 2 schemas: `hdb_catalog` and
    `hdb_views`.
-   (required) Read access to the `information_schema` and `pg_catalog`
    schemas, to query for list of tables.
...
-   (optional) To create tables and views via the Hasura console (the
    ...
    might not be required when you\'re working with an existing
    database.

Here\'s a sample SQL block that you can run on your database to create
