...
use of the \"Schema permissions summary\" page to get a bird\'s eye view
on all the permissions set across all tables and roles. Pay extra
attention to roles like \"anonymous\" which allow unauthenticated
access.

/img/graphql/manual/deployment/schema\_permissions\_summary.png

### Limit number of rows returned

You should
`limit the number of rows <limit-rows-permissions>`{.interpreted-text
role="ref"} that can be accessed in one request, by setting the number
in the select permission. This will prevent someone from accidentally or
otherwise querying the entire table in one shot, thus adding load on
Postgres.
...
`API reference <api_reference>`{.interpreted-text role="ref"}.
...
It is recommended that you disable the console on production
deployments. Also, when you disable the metadata API, console will stop
working.
...
It is recommended that you disable the
`dev mode <dev-mode>`{.interpreted-text role="ref"} on production
deployments for non-admin roles to avoid leaking internal debugging
information to the clients in case of API errors.
...
Read more at `allow_list`{.interpreted-text role="ref"}.
...
See `enable_https`{.interpreted-text role="ref"} for details on
achieving this.

Configure logging
-----------------

The `logs guide <hge_logs>`{.interpreted-text role="ref"} describes
different log types and log levels Hasura GraphQL engine uses. You can
configure the GraphQL engine to enable/disable certain log-types using
the the `--enabled-log-types` flag or the
