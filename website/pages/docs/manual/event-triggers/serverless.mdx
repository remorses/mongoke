...
For this we `setup an event trigger <create_trigger>`{.interpreted-text
role="ref"} on `UPDATE` to the `notes` table which calls an AWS Lambda
function. The AWS Lambda function itself uses a GraphQL mutation to
insert a new row into the `note_revision` table. As the
`event trigger payload <trigger_payload>`{.interpreted-text role="ref"}
in case of updates gives us both the old and the new data, we can store
