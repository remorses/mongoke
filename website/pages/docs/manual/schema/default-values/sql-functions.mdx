...
Let\'s say you want to set the value of some fields as the output of
some custom SQL functions or stored procedures. This is useful to set
values of fields which depend on other fields passed in the input. E.g.
set `submission_time` of an online quiz as 1 hour from the `start_time`.
...
/img/graphql/manual/schema/modify-sql-fn-table.png
...
/img/graphql/manual/schema/create-trigger.png

Step 3: Run an insert mutation
------------------------------

Run a mutation to insert an object with (input = \"yabba dabba doo!\",
output=null) and you\'ll see the output value (output=\"YABBA DABBA
DOO!\") will be set automatically.
...
-   `column_presets`{.interpreted-text role="ref"}
