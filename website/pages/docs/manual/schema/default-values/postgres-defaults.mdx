...
/img/graphql/manual/schema/add-default-value.png
...
Let\'s say we have a field called `roll_number` which we would like to
be set by default as an auto-incremented integer.

Head to `Data -> SQL` and run the following SQL command to create a new
sequence.
