...
/img/graphql/manual/auth/permissions-rule-analogy.png
...
control works similary - you define the following permissions for a
...
`session variables <roles_variables>`{.interpreted-text role="ref"} and
...
`permission_rules`{.interpreted-text role="ref"}.
...
Head to your console and
`create a table <create-tables>`{.interpreted-text role="ref"} called
`author` with the following schema:
...
You\'ll see that this results in a response that contains all the
authors because by default the GraphQL query is accepted with **admin**
permissions.

/img/graphql/manual/auth/fetch-authors.png

### Define access control rules

Now let\'s define an access control rule for the `author` table for a
role `users`. Head to the **Permissions** section of the table (`Data`
\--\> \<table\> \--\> `Permissions` tab) and define permissions as shown
below:

/img/graphql/manual/auth/permission-basics-simple-example.png

This permission rule reads as: \"*For the role* `user` *, table*
`author` *and operation* `select`/`query`, allow access to those rows
...
/img/graphql/manual/auth/permission-basics-query-with-access-control.png
...
only those rows where the value in the `id` column is equal to `1` (*as
indicated by the* `X-Hasura-User-ID` *session variable*).

As described in the
`Introduction to Authentication and Authorization <authorization>`{.interpreted-text
role="ref"} section of the docs, your auth service is required to
resolve authentication tokens into these session variables. See
`Reference - Session Variables<authorization>`{.interpreted-text
role="ref"} for more details.

Next steps
----------

Read about roles and session variables at:
`roles_variables`{.interpreted-text role="ref"}

See more detailed examples at:
`Common access control examples<auth_examples>`{.interpreted-text
role="ref"}
