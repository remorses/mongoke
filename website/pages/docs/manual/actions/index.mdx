...
/img/graphql/manual/actions/actions-arch.png

Support

Actions are supported in Hasura GraphQL engine versions `v.1.2.0` and
above.
...
-   `Response type`: The GraphQL type of the response that the query or
    mutation will return. Actions can only return object types.
...
An action must return an object type. This means, you will have to
define your custom types like so:
...
Read more about `custom types<custom_types>`{.interpreted-text
role="ref"}.
...
Learn more about
`writing an action handler<action_handlers>`{.interpreted-text
role="ref"}.
...
-   **Asynchronous**: `Async actions <async_actions>`{.interpreted-text
    role="ref"} return an `action id` as response to the client before
    receiving a response from the handler and allow the client to
    subscribe to the actual response using the `action id`.
...
GraphQL server yourself, you can use
`remote schemas <remote_schemas>`{.interpreted-text role="ref"} to add
custom types and resolvers.

Learn more
----------

create Custom types \<types/index\> action-handlers async-actions
Codegen \<codegen\> derive action-permissions reuse-types-actions
debugging .. action-examples
