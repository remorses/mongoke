...
-   You can pass an `on_conflict` argument to convert the mutation to an
    `upsert mutation <upsert>`{.interpreted-text role="ref"}.
...
See the
`insert mutation API reference <insert_upsert_syntax>`{.interpreted-text
...
3.  The array relationship objects are inserted at the end. i.e. in this
    ...
    the author\'s `id` collected in the step 2. Hence, it\'s not
    possible to specify `author_id` in the data for the articles
    relationship.

### Many-to-many relationships

Let\'s say the `articles` has a
`many-to-many relationship <many_to_many_modelling>`{.interpreted-text
role="ref"} with the `tags` table via a bridge table `article_tags`.
...
2.  The array relationship objects (from the perspective of `article`)
    ...
    2.  The parent object (from the perspective of `article_tags`) is
        ...
        `tag_id` in the data for the [article\_tags]{.title-ref}
        relationship.
...
`nested upsert caveats <nested-upsert-caveats>`{.interpreted-text
role="ref"}).
...
`default value configuration <postgres_defaults>`{.interpreted-text
role="ref"} i.e. via Postgres defaults or using column presets.
